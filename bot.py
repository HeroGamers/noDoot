...
bot = commands.Bot(command_prefix=os.getenv('prefix'),
                   description="Holding out UserDoots from guilds since June 2019.\n" +
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Game(name="around with new members... [" + os.getenv('prefix') + "help]"))
    ...
    if channel is None:
        logger.logDebug("Channel not found! Returning!")
        return
    ...
    if len(history) == 0:  # no messages
        ...
        vmessage = await channel.send(
            content="Hello there! Welcome to the noDoot Verification Server! Don't worry, you won't be here for long...\n" +
